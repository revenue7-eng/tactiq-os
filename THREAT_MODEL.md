...
release artifacts, the GitHub identity used to authorize CI workflows.

**Current mitigations.** SPDX 2.2 SBOM produced per build (763
packages, 7,178 files at 100% SHA-256 coverage on rc2/rc3). CVE
scanning via `cve-check` against the NIST NVD feed every build. SLSA
v1.0 build-provenance attestation on tagged source archives via
`actions/attest-build-provenance` and Sigstore OIDC keyless signing,
generated by a workflow identity rather than a personal account
(rc3 onwards). Per-file content reproducibility verified at 99.943%
byte-identity on `rootfs.ext4` between independent builds. Kernel
pinned to a specific LTS point-release series. Source archiver retains
upstream tarballs.
