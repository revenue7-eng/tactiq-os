...
| U-Boot → FIT image (kernel + DTB + initramfs) | OFF | PARTIAL | U-Boot is built with FIT signature support (`CONFIG_FIT_SIGNATURE`, `CONFIG_FIT_FULL_CHECK`, merged from `files/fit-signature.cfg`), and the public key of the development signer is injected into the control FDT as a required key. The default boot method is `fit` (`TACTIQ_BOOT_METHOD`, with kernel signing derived from it): `extlinux.conf` names `/fitImage` as both `KERNEL` and `FDT`, so `sysboot` hands the FIT to `bootm` and the device tree comes from the signed configuration, not from `fdt_addr_r`. On the reference development hardware this path verified the configuration signature with the `dev-fit` key for both the kernel and the device tree, and a FIT with one byte of the configuration signature altered in memory was refused. A FIT with no signature at all has not been tested. This does not make the chain verified. The development keys are public by design (`pki/README.md`), so anyone can produce a FIT this bootloader accepts. `extlinux.conf` itself is not verified: pointing it at a raw `Image` sends U-Boot back to `booti`, which loads the kernel without any check. The bootloader holding the key is not verified by the stage below it. Setting `TACTIQ_BOOT_METHOD = "extlinux"` restores the raw `booti` path. Measured boot is enabled (`files/measured-boot.cfg`). U-Boot records the bootloader version string and the device tree into PCR 0 and the kernel command line into PCR 1. On the `bootm` path the kernel is measured into PCR 8: on the reference development hardware PCR 8 equalled one extend of the zero register with the SHA-256 of the deployed `Image`, and PCR 0 was unchanged. On the `booti` path the events labelled `linux` (PCR 8) and `initrd` (PCR 9) hash the event name instead of the data and do not depend on the kernel. Whether SPL verifies `u-boot.itb` itself is a separate question, not established here: SPL carries its own control FDT and this configuration sets no SPL symbols. |
| Kernel measures itself and initramfs | n/a | PARTIAL | Kernel TPM drivers are compiled in (`CONFIG_TCG_*`); IMA is configured at PCRs 10, 11 and 12. What the bootloader extends into PCRs 0–9 before handoff is described in the previous row; on the default `fit` boot path this includes the kernel image (PCR 8). |
