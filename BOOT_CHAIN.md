...
| Kernel runtime → file access enforcement | LOG (dev) / OFF (production) | n/a | `tactiq-image-dev` signs its whole rootfs at build time (`IMAGE_CLASSES += "ima-evm-rootfs"`) and ships the appraisal policy as `/etc/ima/ima-policy`. It boots with `ima_appraise=log`, so violations are recorded and nothing is blocked: `/data/tactiq` carries no signatures, and the policy appraises reads of `tactiq_vault_data_t`. `tactiq-image.bb` does not apply the class — a production image has no signatures and no policy on disk. |
