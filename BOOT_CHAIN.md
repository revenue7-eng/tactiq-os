...
| U-Boot → FIT image (kernel + DTB + initramfs) | OFF | PARTIAL | U-Boot is built with FIT signature support (`CONFIG_FIT_SIGNATURE`, `CONFIG_FIT_FULL_CHECK`, merged from `files/fit-signature.cfg`), and the development signer's public key is injected into the control FDT as a required key. The default boot path does not exercise it: `TACTIQ_BOOT_METHOD` is `extlinux` and `TACTIQ_FIT_SIGN_KERNEL` is `0`, so U-Boot boots a raw kernel image with `booti` and never loads a FIT. On the reference development hardware a signed kernel FIT loaded by hand from the U-Boot prompt passed configuration signature verification with the `dev-fit` key, and the same FIT with one byte of the configuration signature altered in memory was refused (`Failed to verify required signature 'key-dev-fit'`, kernel not loaded). The key is therefore required on the `bootm` path. A FIT with no signature at all has not been tested. This does not make the chain verified: the default `booti` path loads a raw `Image` without any check, so replacing the kernel on the boot partition is not detected, and the bootloader holding the key is itself not verified by the stage below it. This is capability on the `bootm` path, not enforcement of the boot chain. Measured boot is enabled (`files/measured-boot.cfg`). On the default `booti` path U-Boot records the bootloader version string and the device tree into PCR 0 and the kernel command line into PCR 1. The events labelled `linux` (PCR 8) and `initrd` (PCR 9) do not measure the kernel or an initramfs: `booti` leaves the image length unset, and U-Boot then hashes the event name instead of the data, so PCR 8 and PCR 9 hold constants that do not depend on the kernel. Loaded through `bootm` from a FIT, the kernel is measured into PCR 8: on the reference development hardware the recorded digest equalled the SHA-256 of the deployed `Image`, and PCR 0 and PCR 1 were unchanged. Whether SPL verifies `u-boot.itb` itself is a separate question, not established here: SPL carries its own control FDT and this configuration sets no SPL symbols. |
| Kernel measures itself and initramfs | n/a | PARTIAL | Kernel TPM drivers are compiled in (`CONFIG_TCG_*`); IMA is configured at PCRs 10, 11 and 12. What the bootloader extends into PCRs 0–9 before handoff is described in the previous row; on the default boot path the kernel image is not part of it. |
